...
1. Enter the following into the **Connect to Server** dialog:
    ...
    - **Encryption:** Optional **(6)**
    - **Trust server certificate:** Check this box **(7)**
    - Select **Connect (8)**.
...
1. Expand **Security (1)** > **Users (2)** to confirm that the database user was migrated as well **(3)**.
    ...
    > **Note**: You are finished with the SQLVM. Close the remote desktop session and continue from the **WebVM** — the rest of this exercise is done there.
...
1. On your **WebVM**, click the **Start** button.
...
1. Once the Integration Runtime (Self-hosted) node has been **registered successfully**, minimize the Integration Runtime window.
